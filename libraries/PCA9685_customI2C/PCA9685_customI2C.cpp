...
Modified by Soon Kiat Lau 2018 to use a 3rd-party I2C library to
avoid the blocking issues encountered by the Arduino Wire library.
...
PCA9685::PCA9685(I2C& i2cWire, PCA9685_PhaseBalancer phaseBalancer) {
  _i2cWire = &i2cWire;
#else
...
void PCA9685::resetDevices() {
  ...
  _i2cWire->write(0x00, (int) PCA9685_SW_RESET);
...
PCA9685_STATUS PCA9685::init(byte i2cAddress, byte mode) {
  if (_isProxyAddresser) return PCA9685_STATUS_PROXY;
  ...
  if (writeRegister(PCA9685_MODE1_REG, PCA9685_MODE_RESTART | PCA9685_MODE_AUTOINC) != PCA9685_STATUS_OK
      ||
      writeRegister(PCA9685_MODE2_REG, mode) != PCA9685_STATUS_OK)
  {
    return getI2CError();
  }
  else
...
PCA9685_STATUS PCA9685::setPWMFrequency(float pwmFrequency) {
  if (pwmFrequency < 0 || _isProxyAddresser) return PCA9685_STATUS_PROXY;
  ...
  if (readRegister(PCA9685_MODE1_REG, mode1Reg) != PCA9685_STATUS_OK)
  {
    return getI2CError();
  }

  if (writeRegister(PCA9685_MODE1_REG, (mode1Reg = (mode1Reg & ~PCA9685_MODE_RESTART) | PCA9685_MODE_SLEEP)) != PCA9685_STATUS_OK
      ||
      writeRegister(PCA9685_PRESCALE_REG, (byte)preScalerVal) != PCA9685_STATUS_OK
      ||
      // It takes 500us max for the oscillator to be up and running once SLEEP bit has been set to logic 0.
      writeRegister(PCA9685_MODE1_REG, (mode1Reg = (mode1Reg & ~PCA9685_MODE_SLEEP) | PCA9685_MODE_RESTART)) != PCA9685_STATUS_OK)
  {
    return getI2CError();
  }
...
PCA9685_STATUS PCA9685::setChannelOn(int channel) {
  if (channel < 0 || channel > 15) return PCA9685_STATUS_CHANNEL;

#ifdef PCA9685_ENABLE_DEBUG_OUTPUT
  Serial.println("PCA9685::setChannelOn");
#endif

  // time_on = FULL; time_off = 0;
  return writeChannelPWM(getRegAddress(channel), PCA9685_PWM_FULL, 0);
}

PCA9685_STATUS PCA9685::setChannelOff(int channel) {
  if (channel < 0 || channel > 15) return PCA9685_STATUS_CHANNEL;
  ...
  // time_on = 0; time_off = FULL;
  return writeChannelPWM(getRegAddress(channel), 0, PCA9685_PWM_FULL);
}

// pwmAmount: Min is 0, max is PCA9685_PWM_FULL (4096)
PCA9685_STATUS PCA9685::setChannelPWM(int channel, uint16_t pwmAmount) {
  if (channel < 0 || channel > 15) return PCA9685_STATUS_CHANNEL;
  ...
  return writeChannelPWM(getRegAddress(channel), phaseBegin, phaseEnd);
}

// begChannel is the channel at which to begin this iterative process
PCA9685_STATUS PCA9685::setChannelsPWM(int begChannel, int numChannels, const uint16_t *pwmAmounts) {
  if (begChannel < 0 || begChannel > 15 || numChannels < 0) return PCA9685_STATUS_CHANNEL;
  ...
  // In avr/libraries/Wire.h and avr/libraries/utility/twi.h, BUFFER_LENGTH (replaced with 32 here) controls
  ...
  while (numChannels > 0) {
    int maxChannels = min(numChannels, (32 - 1) / 4);
    while (maxChannels-- > 0) {
      uint16_t phaseBegin, phaseEnd;
      getPhaseCycle(begChannel++, *pwmAmounts++, &phaseBegin, &phaseEnd);

      // Subtract one from begChannel to account for how the original code started transmission with
      // begChannel before the ++ operator was applied.
      PCA9685_STATUS status = writeChannelPWM(begChannel - 1, phaseBegin, phaseEnd);
...
byte PCA9685::getRegAddress(int channel) {
  byte regAddress;
...
#ifdef PCA9685_ENABLE_DEBUG_OUTPUT
  Serial.print("  PCA9685::getRegAddress channel: ");
  Serial.print(channel);
  Serial.print(", regAddress: 0x");
  Serial.println(regAddress, HEX);
#endif
...
PCA9685_STATUS PCA9685::writeChannelPWM(byte regAddress, uint16_t phaseBegin, uint16_t phaseEnd) {
#ifdef PCA9685_ENABLE_DEBUG_OUTPUT
  ...
  uint8_t writeBuffer[4] = {  lowByte(phaseBegin) , 
                              highByte(phaseBegin) , 
                              lowByte(phaseEnd) , 
                              highByte(phaseEnd) };

  return i2cWire_write(regAddress, writeBuffer, 4);
}

PCA9685_STATUS PCA9685::writeRegister(byte regAddress, byte value) {
#ifdef PCA9685_ENABLE_DEBUG_OUTPUT
  ...
  return i2cWire_write(regAddress, value);
...
PCA9685_STATUS PCA9685::readRegister(byte regAddress, byte& readBuffer) {
  PCA9685_STATUS status = i2cWire_write(regAddress);
  ...
  status = i2cWire_read(1);

  if (status == PCA9685_STATUS_OK)
  {
    readBuffer = _i2cWire->receive();
  }
...
PCA9685_STATUS PCA9685::i2cWire_read(uint8_t bytesCount)
{
  _lastI2CError = _i2cWire->read(_i2cAddress, bytesCount);

  if (_lastI2CError == I2C_STATUS_OK)
  {
    return PCA9685_STATUS_OK;
  }
  else
...
PCA9685_STATUS PCA9685::i2cWire_write(uint8_t regAddress)
{
  _lastI2CError = _i2cWire->write(_i2cAddress, regAddress);
...
PCA9685_STATUS PCA9685::i2cWire_write(uint8_t regAddress, uint8_t value)
{
  _lastI2CError = _i2cWire->write(_i2cAddress, regAddress, value);
...
PCA9685_STATUS PCA9685::i2cWire_write(uint8_t regAddress, uint8_t value[], uint8_t arrayLen)
{
  _lastI2CError = _i2cWire->write(_i2cAddress, regAddress, value, arrayLen);
...
PCA9685_STATUS PCA9685::getI2CError() {
  switch (_lastI2CError) {
  case I2C_STATUS_START:
    return PCA9685_STATUS_I2C_START;
    break;
  case I2C_STATUS_ACKNACK_MODE:
    return PCA9685_STATUS_I2C_ACKNACK_MODE;
    break;
  case I2C_STATUS_ACKNACK:
    return PCA9685_STATUS_I2C_ACKNACK;
    break;
  case I2C_STATUS_REPSTART:
    return PCA9685_STATUS_I2C_REPSTART;
    break;
  case I2C_STATUS_ACKNACK_RECMODE:
    return PCA9685_STATUS_I2C_ACKNACK_RECMODE;
    break;
  case I2C_STATUS_ACKNACK_REC:
    return PCA9685_STATUS_I2C_ACKNACK_REC;
    break;
  case I2C_STATUS_STOP:
    return PCA9685_STATUS_I2C_STOP;
    break;
  case I2C_STATUS_OTHER:
    return PCA9685_STATUS_I2C_OTHER;
  default:
    return PCA9685_STATUS_OTHER;
    break;
  }
}
